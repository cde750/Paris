from prix_loader import (
    charger_prix, prix_par_quartier, table_saisie_vide,
    bloc_qualite, PrixDataError,
)
...
with tab_invest:
    ...
    st.info("""
    **Lecture des résultats** — Le loyer est *plafonné par arrêté* (donnée
    exacte, opposable). Le prix d'acquisition provient des **transactions
    réelles DVF** agrégées par quartier ou arrondissement, avec une dispersion
    intra-zone importante. Les rendements sont des **ordres de grandeur** :
    la *hiérarchie entre secteurs* est informative, pas la valeur absolue.
    Chaque ligne indique son niveau d'agrégation et son effectif.
    """)
    ...
    H = HypothesesInvest(
       ...
       # ─────────────────── PRIX D'ACQUISITION ───────────────────
    st.divider()
    st.markdown("### 🏷️ Source des prix d'acquisition")

    cp1, cp2 = st.columns([2, 1])
    with cp1:
        src_prix = st.radio(
            "Mode",
            ["DVF officiel (Etalab)", "Saisie manuelle"],
            index=0, horizontal=True,
            help="DVF = transactions réelles publiées par la DGFiP. "
                 "La saisie manuelle vous permet d'utiliser votre propre "
                 "source (Notaires du Grand Paris, etc.) en la citant.",
        )
    with cp2:
        scenario = st.radio(
            "Scénario de prix", ["bas", "median", "haut"], index=1,
    ...
    if src_prix.startswith("DVF"):
        millesimes = st.multiselect(
            "Millésimes DVF",
            [2020, 2021, 2022, 2023, 2024, 2025],
            default=[2023, 2024],
            help="Plusieurs années = échantillon plus large, mais moins "
                 "représentatif du marché actuel.",
        )
        if not millesimes:
            st.warning("Sélectionnez au moins un millésime.")
            st.stop()

        try:
            with st.spinner("Téléchargement et nettoyage des données DVF…"):
                res_prix = charger_prix(tuple(sorted(millesimes)),
                                        gdf_quartiers=None)
            bloc_qualite(res_prix)
            prix_q = prix_par_quartier(agg, res_prix)
        except PrixDataError as e:
            st.error(
                f"**Chargement DVF impossible.**\n\n{e}\n\n"
                "Aucun prix de substitution n'est généré : "
                "basculez en saisie manuelle pour poursuivre."
            )
            st.stop()

    else:
        st.warning(
            "**Saisie manuelle.** Renseignez les prix au m² depuis une source "
            "que vous documentez. Les lignes vides seront exclues des calculs. "
            "Colonnes attendues : P25 (bas) / médiane / P75 (haut)."
        )
        source_citee = st.text_input(
            "Source de vos chiffres (obligatoire pour traçabilité)",
            placeholder="ex. Notaires du Grand Paris, note de conjoncture T2 2026",
        )
        edited = st.data_editor(
            table_saisie_vide(agg),
            num_rows="fixed", use_container_width=True, height=420,
            column_config={
                "quartier": st.column_config.TextColumn("Quartier"),
                "arrondissement": st.column_config.NumberColumn("Arr.", format="%d"),
                "prix_m2_bas": st.column_config.NumberColumn(
                    "P25 €/m²", min_value=1000, max_value=60000, step=100),
                "prix_m2_median": st.column_config.NumberColumn(
                    "Médiane €/m²", min_value=1000, max_value=60000, step=100),
                "prix_m2_haut": st.column_config.NumberColumn(
                    "P75 €/m²", min_value=1000, max_value=60000, step=100),
            },
            disabled=["quartier", "arrondissement", "id_zone", "n_ventes",
                      "niveau_agregation", "fiable", "source_prix", "millesimes"],
        )
        if source_citee.strip():
            edited["source_prix"] = f"Saisie : {source_citee.strip()}"
        prix_q = edited

        n_saisis = int(prix_q["prix_m2_median"].notna().sum())
        if n_saisis == 0:
            st.info("Renseignez au moins un quartier pour lancer les calculs.")
            st.stop()
        st.caption(f"{n_saisis} quartier(s) renseigné(s) sur {len(prix_q)}.")

    # ─────────── Jointure loyers × prix, avec exclusion des NaN ───────────
    base_inv = prix_q.merge(
        agg[["quartier", "loyer_ref", "loyer_max", "loyer_min"]],
        on="quartier", how="inner",
    )

    col_scenario = {"bas": "prix_m2_bas",
                    "median": "prix_m2_median",
                    "haut": "prix_m2_haut"}[scenario]

    n_avant = len(base_inv)
    base_inv = base_inv.dropna(subset=[col_scenario])
    n_exclus = n_avant - len(base_inv)

    if base_inv.empty:
        st.error(
            "**Aucun quartier ne dispose d'un prix exploitable** pour le "
            f"scénario « {scenario} ». Élargissez les millésimes DVF, "
            "changez de scénario, ou saisissez les prix manuellement."
        )
        st.stop()

    if n_exclus:
        st.caption(
            f"⚠️ {n_exclus} quartier(s) exclu(s) des calculs "
            "faute de prix disponible pour ce scénario."
        )
        res = analyser_secteurs(base_inv, surface, complement, H, scenario)

    # Report des métadonnées de qualité dans les résultats
    for c in ["niveau_agregation", "fiable", "n_ventes", "source_prix"]:
        if c in base_inv.columns and c not in res.columns:
            res = res.merge(base_inv[["quartier", c]], on="quartier", how="left")

    # Bandeau de fiabilité
    if "fiable" in res.columns:
        n_fiab = int(res["fiable"].fillna(False).sum())
        n_quart = int((res.get("niveau_agregation") == "quartier").sum()) \
            if "niveau_agregation" in res.columns else 0
        f1, f2, f3 = st.columns(3)
        f1.metric("Secteurs analysés", f"{len(res)}")
        f2.metric("Dont échantillon suffisant", f"{n_fiab}/{len(res)}")
        f3.metric("Dont maille quartier", f"{n_quart}/{len(res)}",
                  help="Les autres utilisent la médiane de leur arrondissement.")
        if n_fiab < len(res):
            st.caption(
                f"⚠️ {len(res) - n_fiab} secteur(s) reposent sur un échantillon "
                "sous le seuil de significativité. À interpréter avec prudence."
            )
    ...
    cols_table = ["quartier", "arrondissement", "prix_m2", "loyer_m2",
                  "cout_total", "loyer_mensuel", "charges_an", "mensualite",
                  "impot_an", "rdt_brut", "rdt_net", "rdt_net_net",
                  "cashflow_mens", "rdt_fonds_propres"]
    cols_trace = [c for c in ["niveau_agregation", "n_ventes", "fiable"]
                  if c in res.columns]

    table = (res.sort_values(tri, ascending=False)
             .head(top_n)[cols_table + cols_trace])
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "rdt_fonds_propres": st.column_config.NumberColumn(
                "Rdt FP %", format="%.1f"),
            "niveau_agregation": st.column_config.TextColumn(
                "Maille prix",
                help="Niveau auquel le prix a été calculé : quartier "
                     "(fin) ou arrondissement (approximatif)."),
            "n_ventes": st.column_config.NumberColumn(
                "N ventes", format="%d",
                help="Nombre de transactions DVF retenues."),
            "fiable": st.column_config.CheckboxColumn(
                "Échant. suffisant",
                help="Vrai si l'effectif dépasse le seuil de significativité."),
        },
    )
    ...
    with g1:
        ...
        fig = px.scatter(
        res, x="prix_m2", y="rdt_net_net",
        color="arrondissement", hover_name="quartier",
        symbol="fiable" if "fiable" in res.columns else None,
        hover_data={"n_ventes": True, "niveau_agregation": True}
                   if "n_ventes" in res.columns else None,
        labels={"prix_m2": "Prix €/m²", "rdt_net_net": "Rdt net-net %"},
    )
        # Iso-rendement brut
